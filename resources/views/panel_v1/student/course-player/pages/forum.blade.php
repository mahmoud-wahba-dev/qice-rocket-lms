...
<div class="flex flex-col gap-8 pb-8">
    ...
    <form method="POST" action="{{ route('panel.v1.student.course.forum.store', ['slug' => $slug ?? 'demo']) }}"
        class="border border-d9 rounded-20px bg-white px-5 sm:px-6 py-5">
        @csrf
        <input type="text" name="title" required maxlength="255" placeholder="عنوان السؤال"
            class="input input-bordered w-full h-12 rounded-10px border-d9 font-semibold text-15px text-black focus:outline-none focus:border-primary mb-3">
        <textarea rows="4" name="message" required maxlength="5000"
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-4 border-t border-d9 mt-2">
            <div class="flex flex-wrap items-center gap-4">
                <span class="inline-flex items-center gap-2 font-medium text-13px text-gray">
                    <span class="icon-[tabler--at] size-5"></span>
                    سيظهر سؤالك باسمك
                </span>
            </div>
            <button type="submit" class="btn btn-primary rounded-10px h-11 px-5 font-bold text-14px gap-2">
    ...
    <div class="flex flex-col gap-5">
        @foreach ($forumData['posts'] as $post)            <article class="border border-d9 rounded-20px bg-white px-5 sm:px-6 py-5">
