...
@php
    ...
    $openCreate = request()->boolean('create') || (isset($errors) && $errors->any());
@endphp
...
<div class="space-y-8 sm:space-y-10 pb-10" data-instructor-support>
    ...
    <div data-support-panel="create" class="{{ $openCreate ? '' : 'hidden' }}">
        <div class="grid grid-cols-1 lg:grid-cols-12 overflow-hidden rounded-20px border border-d9 bg-white shadow-[0_8px_30px_rgba(15,76,69,0.08)]">
            <div class="lg:col-span-8 px-6 sm:px-8 py-8 lg:px-12 lg:py-12 order-2 lg:order-1">
                ...
                <form class="space-y-7 sm:space-y-8" action="{{ route('panel.v1.instructor.support.store') }}" method="POST">
                    @csrf
                    <div class="relative">
                        ...
                        <select id="instructor-support-type" name="department_id" required
                            ...
                            <option value="" disabled {{ old('department_id') ? '' : 'selected' }}>النوع</option>
                    ...
                    <div class="relative">
                        ...
                        <input id="instructor-support-subject" name="title" type="text" required
                            value="{{ old('title') }}"
                    ...
                    <div class="relative">
                        ...
                        <textarea id="instructor-support-message" name="message" rows="8" required
                            ...
                            placeholder="اكتب تفاصيل المشكلة هنا...">{{ old('message') }}</textarea>
                    </div>
    ...
    <div data-support-panel="list" class="{{ $openCreate ? 'hidden' : '' }}">
        <nav class="tabs tabs-bordered flex w-full overflow-x-auto mb-6 sm:mb-8 border-b border-d9" role="tablist">
            <button type="button"
                ...
                data-support-tab="tickets" role="tab" aria-selected="true">تذاكري ({{ count($supportTickets ?? []) }})</button>
            <button type="button"
                ...
                data-support-tab="courses" role="tab" aria-selected="false">دعم الدورات ({{ count($courseSupportRows ?? []) }})</button>
        </nav>
        ...
        <div data-support-tab-panel="tickets">
            ...
            <div class="space-y-3">
                <div class="hidden lg:grid grid-cols-12 gap-3 px-5 py-2 font-semibold text-14px text-gray">
                    ...
                    <div class="col-span-3">الموضوع</div>
                    <div class="col-span-2">القسم</div>
                ...
                @forelse ($supportTickets ?? [] as $index => $ticket)
                    <article class="rounded-14px border border-d9 bg-white px-4 sm:px-5 py-4 sm:py-5 shadow-sm">
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-3 lg:gap-4 lg:items-center">
                            ...
                            <div class="lg:col-span-2">
                                <p class="lg:hidden font-medium text-12px text-gray mb-1">القسم</p>
                                <p class="font-medium text-14px sm:text-15px text-gray">{{ $ticket['department'] ?? '—' }}</p>
                            </div>
                            <div class="lg:col-span-2">
                ...
                <table class="table w-full text-15px sm:text-16px">
                    <thead>
                        <tr class="border-b border-d9 text-gray bg-f9">
                            ...
                            <th class="px-5 py-4 text-start font-semibold">الموضوع</th>
                            <th class="px-5 py-4 text-start font-semibold">الحالة</th>
                            <th class="px-5 py-4 text-start font-semibold">التاريخ</th>
                    ...
                    <tbody>
                        @forelse ($courseSupportRows ?? [] as $row)
                            <tr class="border-b border-d9 last:border-0">
                                ...
                                <td class="px-5 py-5 font-medium text-primary">{{ $row['title'] }}</td>
                                <td class="px-5 py-5">
                                    <span class="inline-flex rounded-full bg-[#ECFDF5] px-3 py-1.5 font-semibold text-13px text-[#059669]">
                                        {{ $row['status'] }}
                                    </span>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-5 py-20 text-center font-medium text-16px text-gray">
