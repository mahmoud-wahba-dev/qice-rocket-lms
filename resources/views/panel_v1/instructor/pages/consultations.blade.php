...
<div class="space-y-6 pb-8">
    ...
    <article class="border border-d9 rounded-14px bg-white p-5">
        <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
            <span class="inline-flex rounded-full bg-[#DBEAFE] px-3 py-1 font-semibold text-12px text-[#1D4ED8]">
                {{ $session['status'] ?? '' }}
            </span>
            <p class="font-semibold text-18px text-primary">{{ $session['price'] ?? '' }}</p>
        </div>
        <h2 class="font-semibold text-20px text-black mb-3">{{ $session['title'] ?? '' }}</h2>
        ...
        <div class="flex flex-wrap gap-2">
            <button type="button"
                class="inline-flex items-center justify-center rounded-10px border border-d9 px-4 h-10 font-semibold text-13px text-primary bg-white hover:bg-fa transition">التفاصيل</button>
            <button type="button"
                class="inline-flex items-center justify-center rounded-10px border border-d9 px-4 h-10 font-semibold text-13px text-primary bg-white hover:bg-fa transition">إلغاء اللقاء</button>
        </div>
    </article>
    ...
    <div class="border border-d9 rounded-14px bg-white overflow-x-auto">
        <table class="table w-full text-13px sm:text-14px">
            <thead>
                <tr class="bg-f9 border-b border-d9 text-gray">
