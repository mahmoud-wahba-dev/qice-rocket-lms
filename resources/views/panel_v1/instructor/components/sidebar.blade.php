@php
    ...
    $navSections = [
        ...
        [
            ...
            'items' => [
                ['label' => 'المحفظة والمبيعات', 'icon' => 'tabler--wallet', 'route' => 'panel.v1.instructor.finance', 'active' => $is('panel.v1.instructor.finance')],
                ['label' => 'المستحقات والسحب', 'icon' => 'tabler--cash', 'route' => 'panel.v1.instructor.payouts', 'active' => $is('panel.v1.instructor.payouts')],
                ['label' => 'التسويق والقسائم', 'icon' => 'tabler--ticket', 'route' => 'panel.v1.instructor.marketing', 'active' => $is('panel.v1.instructor.marketing')],
            ],
        ],
        [
            'title' => 'قسم الإعدادات والدعم',
            'items' => [
                ['label' => 'الإعدادات', 'icon' => 'tabler--settings', 'route' => 'panel.v1.instructor.settings', 'active' => $is('panel.v1.instructor.settings')],
                ['label' => 'المساعدة والدعم', 'icon' => 'tabler--help-circle', 'route' => 'panel.v1.instructor.support', 'active' => $is('panel.v1.instructor.support')],
                ['label' => 'اللوحة القديمة', 'icon' => 'tabler--layout-dashboard', 'href' => '/panel', 'active' => false],
                ['label' => 'تسجيل الخروج', 'icon' => 'tabler--logout', 'href' => '/logout', 'active' => false, 'danger' => true],
            ],
        ],
    ];
@endphp
                        ...
                        @foreach ($section['items'] as $item)
                            ...
                            <li>
                                <a href="{{ $href }}"
                                    ...
                                    <span class="icon-[{{ $item['icon'] }}] size-5 shrink-0 {{ $danger ? 'text-[#EF4444]' : 'text-white' }}"></span>
