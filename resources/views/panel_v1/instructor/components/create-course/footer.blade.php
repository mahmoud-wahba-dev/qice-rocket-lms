...
<div class="sticky bottom-0 z-30 -mx-4 sm:-mx-6 lg:-mx-8 mt-8 border-t border-d9 bg-white/95 backdrop-blur px-4 sm:px-6 lg:px-8 py-4" data-wizard-footer>
    <div class="flex flex-wrap items-center justify-between gap-3 sm:gap-4">
        ...
        <p class="font-medium text-14px sm:text-15px text-gray order-1 sm:order-2 w-full sm:w-auto text-center" data-wizard-footer-progress>
        ...
        <div class="flex items-center gap-2.5 sm:gap-3 order-3 ms-auto sm:ms-0">
            <button type="button" data-wizard-prev
                class="inline-flex items-center gap-1.5 h-11 sm:h-12 px-4 sm:px-5 rounded-10px border border-d9 bg-white font-semibold text-15px sm:text-16px text-primary hover:bg-[#FAFAF4] transition {{ $step > 1 ? '' : 'opacity-60 cursor-not-allowed' }}"
                {{ $step > 1 ? '' : 'disabled' }}>
            ...
            <button type="button" data-wizard-next
                ...
                <span data-wizard-next-label>{{ $meta['next'] ?? 'التالي' }}</span>
                <span class="icon-[tabler--chevron-left] size-5" data-wizard-next-icon></span>
            </button>
        </div>
    </div>
</div>
