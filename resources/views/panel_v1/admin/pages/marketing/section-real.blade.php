                ...
                <table class="table w-full text-13px">
                    ...
                    @forelse ($discounts as $d)
                        ...
                        <tr class="border-b border-d9 last:border-0">
                            ...
                            <td class="px-3 py-3 text-center"><div class="flex items-center justify-center gap-1.5"><a href="{{ route('panel.v1.admin.marketing.discounts.edit',['id'=>$d->id]) }}" class="size-7 rounded-8px border border-d9 center hover:bg-[#FAFAF4]" title="تعديل"><span class="icon-[tabler--edit] size-3.5 text-primary"></span></a><form method="POST" action="{{ route('panel.v1.admin.marketing.discounts.delete',['id'=>$d->id]) }}" onsubmit="return confirm('حذف القسيمة؟')" class="inline">@csrf<button type="submit" class="size-7 rounded-8px border border-red-200 center hover:bg-red-50" title="حذف"><span class="icon-[tabler--trash] size-3.5 text-red-500"></span></button></form></div></td>
                        </tr>
                    @empty
    ...
    @isset($cashbacks)
        <div class="border border-d9 rounded-14px bg-white overflow-hidden">
            ...
            <div class="overflow-x-auto"><table class="table w-full text-14px"><thead><tr class="bg-fa border-b border-d9 text-gray"><th class="px-4 py-3 text-start">#</th><th class="px-4 py-3 text-start">القاعدة</th><th class="px-4 py-3 text-center">النسبة/المبلغ</th><th class="px-4 py-3 text-center">الحالة</th><th class="px-4 py-3 text-center">إجراء</th></tr></thead><tbody>
                @forelse($cashbacks as $c)<tr class="border-b border-d9 last:border-0"><td class="px-4 py-3 font-bold text-primary">#{{ $c->id }}</td><td class="px-4 py-3 font-medium text-primary">{{ $c->title ?? 'قاعدة #'.$c->id }}</td><td class="px-4 py-3 text-center font-semibold text-primary">{{ $c->amount ?? '—' }} {{ $c->amount_type ?? '' }}</td><td class="px-4 py-3 text-center"><span class="inline-flex rounded-full px-3 py-1 font-semibold text-11px {{ $c->enable ? 'bg-[#D1FAE5] text-[#059669]' : 'bg-[#FEE2E2] text-[#DC2626]' }}">{{ $c->enable ? 'نشط' : 'معطل' }}</span></td><td class="px-4 py-3 text-center"><div class="flex items-center justify-center gap-1.5"><form method="POST" action="{{ route('panel.v1.admin.marketing.cashback.toggle',['id'=>$c->id]) }}" class="inline">@csrf<button type="submit" class="size-8 rounded-8px bg-[#D1FAE5] center hover:opacity-90" title="تفعيل/تعطيل"><span class="icon-[tabler--repeat] size-4 text-[#059669]"></span></button></form><form method="POST" action="{{ route('panel.v1.admin.marketing.cashbacks.delete',['id'=>$c->id]) }}" onsubmit="return confirm('حذف؟')" class="inline">@csrf<button type="submit" class="size-8 rounded-8px border border-red-200 center" title="حذف"><span class="icon-[tabler--trash] size-4 text-red-500"></span></button></form></div></td></tr>@empty<tr><td colspan="5" class="px-4 py-14 text-center font-medium text-gray">لا توجد قواعد استرداد — تُنشأ من لوحة CashbackRuleController القديمة (target_type/category/product...)</td></tr>@endforelse
            </tbody></table></div>
        </div>
    @endisset
    ...
    @isset($points)
        <div class="border border-d9 rounded-14px bg-white overflow-hidden">
            ...
            <div class="overflow-x-auto"><table class="table w-full text-14px"><thead><tr class="bg-fa border-b border-d9 text-gray"><th class="px-4 py-3 text-start">#</th><th class="px-4 py-3 text-start">المستخدم</th><th class="px-4 py-3 text-center">النقاط</th><th class="px-4 py-3 text-center">النوع</th><th class="px-4 py-3 text-center">التاريخ</th><th class="px-4 py-3 text-center">إجراء</th></tr></thead><tbody>
                @forelse($points as $p)<tr class="border-b border-d9 last:border-0"><td class="px-4 py-3 font-bold text-primary">#{{ $p->id }}</td><td class="px-4 py-3 font-medium text-primary">{{ $p->user->full_name ?? $p->user_id }}</td><td class="px-4 py-3 text-center font-bold text-primary">{{ $p->score }}</td><td class="px-4 py-3 text-center font-medium text-gray">{{ $p->item ?? $p->type }}</td><td class="px-4 py-3 text-center font-medium text-gray">{{ date('Y/m/d', (int)$p->created_at) }}</td><td class="px-4 py-3 text-center"><form method="POST" action="{{ route('panel.v1.admin.marketing.points.delete',['id'=>$p->id]) }}" onsubmit="return confirm('حذف؟')" class="inline">@csrf<button type="submit" class="size-8 rounded-8px border border-red-200 center" title="حذف"><span class="icon-[tabler--trash] size-4 text-red-500"></span></button></form></td></tr>@empty<tr><td colspan="6" class="px-4 py-14 text-center font-medium text-gray">لا توجد حركات نقاط بعد — تُسجل تلقائياً عند الإنجاز/الشراء</td></tr>@endforelse
            </tbody></table></div>
        </div>
    @endisset
    ...
    @isset($gifts)
        <div class="border border-d9 rounded-14px bg-white overflow-hidden">
            ...
            <div class="overflow-x-auto"><table class="table w-full text-14px"><thead><tr class="bg-fa border-b border-d9 text-gray"><th class="px-4 py-3 text-start">#</th><th class="px-4 py-3 text-start">المرسل</th><th class="px-4 py-3 text-center">المستلم</th><th class="px-4 py-3 text-center">الحالة</th><th class="px-4 py-3 text-center">التاريخ</th><th class="px-4 py-3 text-center">إجراء</th></tr></thead><tbody>
                @forelse($gifts as $g)<tr class="border-b border-d9 last:border-0"><td class="px-4 py-3 font-bold text-primary">#{{ $g->id }}</td><td class="px-4 py-3 font-medium text-primary">{{ $g->user->full_name ?? $g->user_id }}</td><td class="px-4 py-3 text-center font-medium text-primary">{{ $g->email ?? $g->receipt->email ?? '—' }}</td><td class="px-4 py-3 text-center"><span class="inline-flex rounded-full px-3 py-1 font-semibold text-11px bg-[#D1FAE5] text-[#059669]">{{ $g->status }}</span></td><td class="px-4 py-3 text-center font-medium text-gray">{{ date('Y/m/d', (int)$g->created_at) }}</td><td class="px-4 py-3 text-center"><div class="flex items-center justify-center gap-1.5"><form method="POST" action="{{ route('panel.v1.admin.marketing.gifts.reminder',['id'=>$g->id]) }}" class="inline">@csrf<button type="submit" class="size-8 rounded-8px bg-[#EFF6FF] center hover:opacity-90" title="تذكير"><span class="icon-[tabler--bell] size-4 text-[#2563EB]"></span></button></form><form method="POST" action="{{ route('panel.v1.admin.marketing.gifts.cancel',['id'=>$g->id]) }}" onsubmit="return confirm('إلغاء الهدية مع الاسترداد؟')" class="inline">@csrf<button type="submit" class="size-8 rounded-8px bg-[#FEE2E2] center hover:opacity-90" title="إلغاء"><span class="icon-[tabler--x] size-4 text-[#DC2626]"></span></button></form><form method="POST" action="{{ route('panel.v1.admin.marketing.gifts.delete',['id'=>$g->id]) }}" onsubmit="return confirm('حذف؟')" class="inline">@csrf<button type="submit" class="size-8 rounded-8px border border-red-200 center" title="حذف"><span class="icon-[tabler--trash] size-4 text-red-500"></span></button></form></div></td></tr>@empty<tr><td colspan="6" class="px-4 py-14 text-center font-medium text-gray">لا توجد هدايا — تُنشأ عند إهداء دورة/حزمة عبر GiftController</td></tr>@endforelse
            </tbody></table></div>
        </div>
    @endisset
    ...
    @isset($pages)
        <div class="border border-d9 rounded-14px bg-white overflow-hidden">
            ...
            <div class="overflow-x-auto"><table class="table w-full text-14px"><thead><tr class="bg-fa border-b border-d9 text-gray"><th class="px-4 py-3 text-start">الصفحة</th><th class="px-4 py-3 text-center">الرابط</th><th class="px-4 py-3 text-center">إجراء</th></tr></thead><tbody>
                @forelse($pages as $pg)<tr class="border-b border-d9 last:border-0"><td class="px-4 py-3 font-bold text-primary">{{ $pg->title ?? 'صفحة #'.$pg->id }}</td><td class="px-4 py-3 text-center font-medium text-gray" dir="ltr">{{ $pg->link ?? $pg->slug ?? '—' }}</td><td class="px-4 py-3 text-center"><form method="POST" action="{{ route('panel.v1.admin.marketing.pages.delete',['id'=>$pg->id]) }}" onsubmit="return confirm('حذف؟')" class="inline">@csrf<button type="submit" class="size-8 rounded-8px border border-red-200 center" title="حذف"><span class="icon-[tabler--trash] size-4 text-red-500"></span></button></form></td></tr>@empty<tr><td colspan="3" class="px-4 py-14 text-center font-medium text-gray">لا توجد صفحات — تُنشأ من PagesController</td></tr>@endforelse
            </tbody></table></div>
        </div>
    @endisset
    ...
    @isset($forms)
        <div class="border border-d9 rounded-14px bg-white overflow-hidden">
            ...
            <div class="overflow-x-auto"><table class="table w-full text-14px"><thead><tr class="bg-fa border-b border-d9 text-gray"><th class="px-4 py-3 text-start">النموذج</th><th class="px-4 py-3 text-center">الحقول</th><th class="px-4 py-3 text-center">التقديمات</th><th class="px-4 py-3 text-center">الرابط</th><th class="px-4 py-3 text-center">إجراءات</th></tr></thead><tbody>
                @forelse($forms as $f)<tr class="border-b border-d9 last:border-0"><td class="px-4 py-3 font-bold text-primary">{{ $f->title ?? 'نموذج #'.$f->id }}</td><td class="px-4 py-3 text-center font-semibold text-primary">{{ $f->fields_count ?? '—' }}</td><td class="px-4 py-3 text-center font-semibold text-primary">{{ $f->submissions_count ?? '—' }}</td><td class="px-4 py-3 text-center font-mono text-12px text-gray" dir="ltr">{{ $f->url }}</td><td class="px-4 py-3 text-center"><div class="flex items-center justify-center gap-1.5"><a href="{{ route('panel.v1.admin.marketing.forms.edit',['id'=>$f->id]) }}" class="size-8 rounded-8px border border-d9 center hover:bg-[#FAFAF4]" title="تعديل"><span class="icon-[tabler--edit] size-4 text-primary"></span></a><form method="POST" action="{{ route('panel.v1.admin.marketing.forms.delete',['id'=>$f->id]) }}" onsubmit="return confirm('حذف؟')" class="inline">@csrf<button type="submit" class="size-8 rounded-8px border border-red-200 center" title="حذف"><span class="icon-[tabler--trash] size-4 text-red-500"></span></button></form></div></td></tr>@empty<tr><td colspan="5" class="px-4 py-14 text-center font-medium text-gray">لا توجد نماذج — أنشئ أول نموذج</td></tr>@endforelse
            </tbody></table></div>
        </div>
    @endisset
    ...
    @isset($floatingBars)
        <div class="border border-d9 rounded-14px bg-white overflow-hidden">
            ...
            <div class="overflow-x-auto"><table class="table w-full text-14px"><thead><tr class="bg-fa border-b border-d9 text-gray"><th class="px-4 py-3 text-start">#</th><th class="px-4 py-3 text-start">العنوان</th><th class="px-4 py-3 text-center">الموضع</th><th class="px-4 py-3 text-center">مفعّل</th><th class="px-4 py-3 text-center">إجراءات</th></tr></thead><tbody>
                @forelse($floatingBars as $fb)<tr class="border-b border-d9 last:border-0"><td class="px-4 py-3 font-bold text-primary">#{{ $fb->id }}</td><td class="px-4 py-3 font-medium text-primary">{{ $fb->title ?? 'شريط #'.$fb->id }}</td><td class="px-4 py-3 text-center font-medium text-gray">{{ $fb->position }}</td><td class="px-4 py-3 text-center"><span class="inline-flex rounded-full px-3 py-1 font-semibold text-11px {{ $fb->enable ? 'bg-[#D1FAE5] text-[#059669]' : 'bg-gray-100 text-gray-500' }}">{{ $fb->enable ? 'مفعّل' : 'معطل' }}</span></td><td class="px-4 py-3 text-center"><div class="flex items-center justify-center gap-1.5"><a href="{{ route('panel.v1.admin.marketing.floating-bars.edit',['id'=>$fb->id]) }}" class="size-8 rounded-8px border border-d9 center hover:bg-[#FAFAF4]" title="تعديل"><span class="icon-[tabler--edit] size-4 text-primary"></span></a><form method="POST" action="{{ route('panel.v1.admin.marketing.floating-bars.delete',['id'=>$fb->id]) }}" onsubmit="return confirm('حذف؟')" class="inline">@csrf<button type="submit" class="size-8 rounded-8px border border-red-200 center" title="حذف"><span class="icon-[tabler--trash] size-4 text-red-500"></span></button></form></div></td></tr>@empty<tr><td colspan="5" class="px-4 py-14 text-center font-medium text-gray">لا يوجد شريط عائم — أنشئ أول شريط</td></tr>@endforelse
            </tbody></table></div>
        </div>
    @endisset
    ...
    @isset($purchaseNotifications)
        <div class="border border-d9 rounded-14px bg-white overflow-hidden">
            ...
            <div class="overflow-x-auto"><table class="table w-full text-14px"><thead><tr class="bg-fa border-b border-d9 text-gray"><th class="px-4 py-3 text-start">العنوان</th><th class="px-4 py-3 text-center">المدة</th><th class="px-4 py-3 text-center">التأخير</th><th class="px-4 py-3 text-center">مفعّل</th><th class="px-4 py-3 text-center">إجراءات</th></tr></thead><tbody>
                @forelse($purchaseNotifications as $pn)<tr class="border-b border-d9 last:border-0"><td class="px-4 py-3 font-bold text-primary">{{ $pn->title ?? 'إشعار #'.$pn->id }}</td><td class="px-4 py-3 text-center font-medium text-gray">{{ $pn->popup_duration }}ث</td><td class="px-4 py-3 text-center font-medium text-gray">{{ $pn->popup_delay }}ث</td><td class="px-4 py-3 text-center"><span class="inline-flex rounded-full px-3 py-1 font-semibold text-11px {{ $pn->enable ? 'bg-[#D1FAE5] text-[#059669]' : 'bg-[#FEE2E2] text-[#DC2626]' }}">{{ $pn->enable ? 'مفعّل' : 'معطل' }}</span></td><td class="px-4 py-3 text-center"><div class="flex items-center justify-center gap-1.5"><a href="{{ route('panel.v1.admin.marketing.purchase-notifications.edit',['id'=>$pn->id]) }}" class="size-8 rounded-8px border border-d9 center hover:bg-[#FAFAF4]" title="تعديل"><span class="icon-[tabler--edit] size-4 text-primary"></span></a><form method="POST" action="{{ route('panel.v1.admin.marketing.purchase-notifications.delete',['id'=>$pn->id]) }}" onsubmit="return confirm('حذف؟')" class="inline">@csrf<button type="submit" class="size-8 rounded-8px border border-red-200 center" title="حذف"><span class="icon-[tabler--trash] size-4 text-red-500"></span></button></form></div></td></tr>@empty<tr><td colspan="5" class="px-4 py-14 text-center font-medium text-gray">لا توجد إشعارات شراء — أنشئ أول إشعار</td></tr>@endforelse
            </tbody></table></div>
        </div>
    @endisset
    ...
    @isset($rules)
        <div class="border border-d9 rounded-14px bg-white overflow-hidden">
            ...
            <div class="overflow-x-auto"><table class="table w-full text-14px"><thead><tr class="bg-fa border-b border-d9 text-gray"><th class="px-4 py-3 text-start">#</th><th class="px-4 py-3 text-start">القاعدة</th><th class="px-4 py-3 text-center">النوع المستهدف</th><th class="px-4 py-3 text-center">الحالة</th><th class="px-4 py-3 text-center">إجراء</th></tr></thead><tbody>
                @forelse($rules as $rl)<tr class="border-b border-d9 last:border-0"><td class="px-4 py-3 font-bold text-primary">#{{ $rl->id }}</td><td class="px-4 py-3 font-medium text-primary">{{ $rl->title ?? 'قاعدة #'.$rl->id }}</td><td class="px-4 py-3 text-center font-medium text-gray">{{ $rl->target_type ?? '—' }}</td><td class="px-4 py-3 text-center"><span class="inline-flex rounded-full px-3 py-1 font-semibold text-11px {{ $rl->enable ? 'bg-[#D1FAE5] text-[#059669]' : 'bg-[#FEE2E2] text-[#DC2626]' }}">{{ $rl->enable ? 'مفعّلة' : 'معطلة' }}</span></td><td class="px-4 py-3 text-center"><div class="flex items-center justify-center gap-1.5"><a href="{{ route('panel.v1.admin.marketing.abandoned-cart.edit',['id'=>$rl->id]) }}" class="size-8 rounded-8px border border-d9 center hover:bg-[#FAFAF4]" title="تعديل"><span class="icon-[tabler--edit] size-4 text-primary"></span></a><form method="POST" action="{{ route('panel.v1.admin.marketing.abandoned-cart.delete',['id'=>$rl->id]) }}" onsubmit="return confirm('حذف؟')" class="inline">@csrf<button type="submit" class="size-8 rounded-8px border border-red-200 center" title="حذف"><span class="icon-[tabler--trash] size-4 text-red-500"></span></button></form></div></td></tr>@empty<tr><td colspan="5" class="px-4 py-14 text-center font-medium text-gray">لا توجد قواعد — تُنشأ لاستهداف السلال المتروكة بالكوبونات</td></tr>@endforelse
            </tbody></table></div>
        </div>
    @endisset
    ...
    @isset($productBadges)
        <div class="border border-d9 rounded-14px bg-white overflow-hidden">
            ...
            <div class="overflow-x-auto"><table class="table w-full text-14px"><thead><tr class="bg-fa border-b border-d9 text-gray"><th class="px-4 py-3 text-start">الشارة</th><th class="px-4 py-3 text-center">المحتوى</th><th class="px-4 py-3 text-center">الحالة</th><th class="px-4 py-3 text-center">إجراءات</th></tr></thead><tbody>
                @forelse($productBadges as $pb)<tr class="border-b border-d9 last:border-0"><td class="px-4 py-3 font-bold text-primary">{{ $pb->title ?? 'شارة #'.$pb->id }}</td><td class="px-4 py-3 text-center font-semibold text-primary">{{ $pb->contents_count ?? '—' }}</td><td class="px-4 py-3 text-center"><span class="inline-flex rounded-full px-3 py-1 font-semibold text-11px {{ $pb->enable ? 'bg-[#D1FAE5] text-[#059669]' : 'bg-[#FEE2E2] text-[#DC2626]' }}">{{ $pb->enable ? 'نشط' : 'معطل' }}</span></td><td class="px-4 py-3 text-center"><div class="flex items-center justify-center gap-1.5"><a href="{{ route('panel.v1.admin.marketing.product-badges.edit',['id'=>$pb->id]) }}" class="size-8 rounded-8px border border-d9 center hover:bg-[#FAFAF4]" title="تعديل"><span class="icon-[tabler--edit] size-4 text-primary"></span></a><form method="POST" action="{{ route('panel.v1.admin.marketing.product-badges.delete',['id'=>$pb->id]) }}" onsubmit="return confirm('حذف؟')" class="inline">@csrf<button type="submit" class="size-8 rounded-8px border border-red-200 center" title="حذف"><span class="icon-[tabler--trash] size-4 text-red-500"></span></button></form></div></td></tr>@empty<tr><td colspan="4" class="px-4 py-14 text-center font-medium text-gray">لا توجد شارات — أنشئ أول شارة</td></tr>@endforelse
            </tbody></table></div>
        </div>
    @endisset
