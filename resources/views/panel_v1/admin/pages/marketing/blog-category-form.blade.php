...
<div class="space-y-6 pb-8 max-w-3xl mx-auto">
    @include('panel_v1.admin.components.page-header', ['title'=> $isEdit ? 'تعديل تصنيف مدونة' : 'تصنيف مدونة جديد','subtitle'=>'منطق Admin\\BlogCategoryController حرفياً — عنوان + slug'])
    <div class="border border-d9 rounded-14px bg-white p-6 sm:p-8">
        <form method="POST" action="{{ $formAction ?? route('panel.v1.admin.marketing.blog-categories.store') }}" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="sm:col-span-2"><label class="font-semibold text-14px text-primary mb-2 block">اسم التصنيف *</label><input type="text" name="title" value="{{ old('title', $category->title ?? '') }}" required class="input input-bordered w-full h-12 rounded-12px border-d9 text-14px">@error('title')<p class="text-red-500 text-12px mt-1">{{ $message }}</p>@enderror</div>
                <div class="sm:col-span-2"><label class="font-semibold text-14px text-primary mb-2 block">الوصف المختصر</label><input type="text" name="subtitle" value="{{ old('subtitle', $category->subtitle ?? '') }}" class="input input-bordered w-full h-12 rounded-12px border-d9 text-14px" placeholder="اختياري">@error('subtitle')<p class="text-red-500 text-12px mt-1">{{ $message }}</p>@enderror</div>
                <div><label class="font-semibold text-14px text-primary mb-2 block">الرابط (slug)</label><input type="text" name="slug" value="{{ old('slug', $category->slug ?? '') }}" dir="ltr" class="input input-bordered w-full h-12 rounded-12px border-d9 text-14px" placeholder="فارغ = تلقائي">@error('slug')<p class="text-red-500 text-12px mt-1">{{ $message }}</p>@enderror</div>
