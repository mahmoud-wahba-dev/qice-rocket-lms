...
<div class="space-y-6 pb-8 max-w-3xl mx-auto">
    @include('panel_v1.admin.components.page-header', [
        'title' => $isEdit ? 'تعديل فصل رائج' : 'إنشاء فصول الرائجة',
        'subtitle' => 'منطق Admin\TrendCategoriesController حرفياً — تصنيف + أيقونة + لون',
    ])
        ...
        <form method="POST" action="{{ $formAction ?? route('panel.v1.admin.education.trends.store') }}" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="sm:col-span-2">
                    ...
                    <select name="category_id" required class="select select-bordered w-full h-12 rounded-12px border-d9 text-14px bg-white">
                        ...
                        @foreach($categories ?? [] as $cat)
                            <option value="{{ $cat->id }}" @selected(old('category_id', $trend->category_id ?? '')==$cat->id)>{{ $cat->title }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="icon" value="{{ old('icon', $trend->icon ?? '') }}" required class="input input-bordered w-full h-12 rounded-12px border-d9 text-14px bg-white" placeholder="icon-[tabler--star]">
                ...
                <div>
                    ...
                    <input type="color" name="color" value="{{ old('color', $trend->color ?? '#0F3D36') }}" required class="input input-bordered w-full h-12 rounded-12px border-d9 bg-white p-1">
            ...
            <div class="flex items-center gap-3 pt-4 border-t border-d9">
                <button type="submit" class="inline-flex items-center justify-center h-12 px-8 rounded-12px bg-primary text-white font-bold text-14px hover:opacity-95 transition">
                    {{ $isEdit ? 'حفظ التعديلات' : 'إضافة الفصل للرائجة' }}
                </button>
