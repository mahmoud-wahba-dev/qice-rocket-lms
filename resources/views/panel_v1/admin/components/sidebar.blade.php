    ...
    <div class="h-full border-e border-white/10 p-0 !bg-primary !text-white">
        <div class="flex h-full max-h-full flex-col !bg-primary !text-white">
            ...
            <nav class="admin-sidebar-scroll flex-1 overflow-y-auto overflow-x-hidden px-3 pb-6" aria-label="تنقل الإدارة">
                @php
                    $isGroupActive = function($group) use ($activeKey) {
                        foreach (($group['items'] ?? []) as $it) if (($it['key'] ?? '') === $activeKey) return true;
                        return false;
                    };
                @endphp
                @foreach ($groups as $idx => $group)
                    @php $open = $isGroupActive($group); @endphp
                    <div data-admin-group data-group-index="{{ $idx }}" class="mb-1">
                        <button type="button"
                            data-admin-group-toggle
                            aria-expanded="{{ $open ? 'true' : 'false' }}"
                            class="w-full flex items-center justify-between gap-2 px-2 py-2 rounded-8px hover:bg-white/10 transition text-start">
                            <span class="{{ $navTitle }} !p-0 flex-1 text-start" data-sidebar-section>{{ $group['title'] }}</span>
                            <span data-admin-group-chevron class="icon-[tabler--chevron-down] size-3.5 shrink-0 text-white/60 transition-transform duration-200 {{ $open ? 'rotate-180' : '' }}"></span>
                        </button>
                        <ul data-admin-group-panel class="flex flex-col gap-1 mt-1 {{ $open ? '' : 'hidden' }}">
