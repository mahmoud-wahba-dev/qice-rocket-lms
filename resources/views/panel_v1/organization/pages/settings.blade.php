    ...
    <div class="container">
        ...
        <div>
            <div id="settings-tabs-1" role="tabpanel" aria-labelledby="settings-tabs-item-1">
                <form action="{{ route('panel.v1.organization.settings.update') }}" method="POST">
                    @csrf
            ...
            <div id="settings-tabs-2" class="hidden" role="tabpanel" aria-labelledby="settings-tabs-item-2">
                <form method="POST" action="{{ route('panel.v1.organization.extra.update') }}">
                    @csrf
            ...
            <div id="settings-tabs-3" class="hidden" role="tabpanel" aria-labelledby="settings-tabs-item-3">
                <form method="POST" action="{{ route('panel.v1.organization.financial.update') }}" enctype="multipart/form-data">
                    @csrf
            ...
            <div id="settings-tabs-4" class="hidden" role="tabpanel" aria-labelledby="settings-tabs-item-4">
                <form method="POST" action="{{ route('panel.v1.organization.images.update') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        @foreach ([
                            ...
                            <div class="{{ $card }}">
                                ...
                                @if (!empty($authUser->$field))
                                    <button type="submit" form="org-delete-media-{{ $field }}" class="mt-4 font-bold text-14px text-[#EF4444]" onclick="return confirm('حذف الملف؟');">حذف الملف الحالي</button>
                                @endif
                            </div>
                        @endforeach
                        <div class="{{ $card }}">
                            ...
                            @if (!empty($signature))
                                <button type="submit" form="org-delete-media-signature_img" class="mt-4 font-bold text-14px text-[#EF4444]" onclick="return confirm('حذف التوقيع؟');">حذف التوقيع</button>
                            @endif
                        </div>
                    </div>
                    <div class="mt-8">
                ...
                @foreach (['avatar', 'cover_img', 'profile_secondary_image', 'profile_video', 'signature_img'] as $mediaType)
                    @if (($mediaType === 'signature_img' && !empty($signature)) || ($mediaType !== 'signature_img' && !empty($authUser->$mediaType)))
                        <form id="org-delete-media-{{ $mediaType }}" method="POST" action="{{ route('panel.v1.organization.media.delete', ['type' => $mediaType]) }}" class="hidden">@csrf</form>
                    @endif
                @endforeach
            </div>

            <div id="settings-tabs-5" class="hidden" role="tabpanel" aria-labelledby="settings-tabs-item-5">
                <form method="POST" action="{{ route('panel.v1.organization.about.update') }}">
                    @csrf
                    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                        <div class="lg:col-span-7 {{ $card }}">
                                ...
                                <div class="space-y-3 mb-6">
                                    @forelse ($educations ?? [] as $education)
                                        <div class="flex items-center justify-between gap-3 border border-d9 rounded-10px px-4 py-3">
                                            ...
                                            <form method="POST" action="{{ route('panel.v1.organization.metas.delete', ['metaId' => $education->id]) }}" onsubmit="return confirm('حذف؟');">
                                                @csrf
                                ...
                                <div class="space-y-3 mb-6">
                                    @forelse ($experiences ?? [] as $experience)
                                        <div class="flex items-center justify-between gap-3 border border-d9 rounded-10px px-4 py-3">
                                            ...
                                            <form method="POST" action="{{ route('panel.v1.organization.metas.delete', ['metaId' => $experience->id]) }}" onsubmit="return confirm('حذف؟');">
                                                @csrf
                                                <button type="submit" class="font-bold text-14px text-[#EF4444]">حذف</button>
                                            </form>
                                        </div>
                                    ...
                                    @forelse ($attachments ?? [] as $attachment)
                                        <div class="flex items-center justify-between gap-3 border border-d9 rounded-10px px-4 py-3">
                                            ...
                                            <div class="flex items-center gap-3 shrink-0">
                                                ...
                                                @endif
                                                <form method="POST" action="{{ route('panel.v1.organization.attachments.delete', ['attachmentId' => $attachment->id]) }}" onsubmit="return confirm('حذف؟');">
                                                    @csrf
                                                    <button type="submit" class="font-bold text-14px text-[#EF4444]">حذف</button>
                                                </form>
                                            </div>
                ...
                <div class="{{ $card }} mt-6">
                    ...
                    <form method="POST" action="{{ route('panel.v1.organization.attachments.store') }}" enctype="multipart/form-data" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        @csrf
                                    ...
                                    <tr class="border-b border-d9 last:border-0">
                                        ...
                                        <td class="px-4 py-4">
                                            @if (empty($session->session_end_at))
                                                <form method="POST" action="{{ route('panel.v1.organization.sessions.end', ['sessionId' => $session->id]) }}" onsubmit="return confirm('إنهاء هذه الجلسة؟');">
                                                    @csrf
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function postMeta(name, inputId) {
        ...
        fetch("{{ route('panel.v1.organization.metas.store') }}", {
            method: 'POST',
