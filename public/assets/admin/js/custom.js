...
(function ($) {
    ...
    function initAdminFileManager($btn) {
        if ($btn.data('filemanager-init')) {
            return;
        }

        $btn.data('filemanager-init', true);
        $btn.filemanager('file', {prefix: '/laravel-filemanager'});
    }

    $(function () {
        $('.admin-file-manager').each(function () {
            initAdminFileManager($(this));
        });
    });
