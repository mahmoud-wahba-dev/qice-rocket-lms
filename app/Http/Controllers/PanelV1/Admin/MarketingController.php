...
class MarketingController extends AdminController
{
    public function home(Request $request)
    {
        ...
        $contentCards = [
            ['title' => 'المتجر', 'icon' => 'icon-[tabler--shopping-cart]', 'section' => 'products'],
            ['title' => 'المدونة', 'icon' => 'icon-[tabler--article]', 'section' => 'blog'],
            ['title' => 'الصفحات', 'icon' => 'icon-[tabler--file-text]', 'section' => 'pages'],
            ['title' => 'القسائم والخصومات', 'icon' => 'icon-[tabler--discount]', 'section' => 'discounts'],
            ['title' => 'العروض الخاصة', 'icon' => 'icon-[tabler--confetti]', 'section' => 'special_offers'],
            ['title' => 'التسويق بالعمولة', 'icon' => 'icon-[tabler--share]', 'section' => 'affiliate'],
            ['title' => 'الاسترداد النقدي', 'icon' => 'icon-[tabler--cash]', 'section' => 'cashback'],
            ['title' => 'نقاط المكافآت', 'icon' => 'icon-[tabler--coin]', 'section' => 'points'],
            ['title' => 'الهدايا', 'icon' => 'icon-[tabler--gift]', 'section' => 'gifts'],
            ['title' => 'مكافأة التسجيل', 'icon' => 'icon-[tabler--gift-card]', 'section' => 'registration-bonus'],
            ['title' => 'لوحة قيادة التسويق', 'icon' => 'icon-[tabler--chart-pie]', 'section' => 'dashboard'],
            ['title' => 'أدوات التسويق', 'icon' => 'icon-[tabler--tool]', 'section' => 'tools'],
        ];
    ...
    public function storeBlogCategory(Request $request){ $u=$this->resolveAdmin($request); if($u instanceof \Illuminate\Http\RedirectResponse) return $u; $request->validate(['title'=>'required']); $data=$request->all(); $cat=\App\Models\BlogCategory::create(['slug'=>\Illuminate\Support\Str::slug($data['title']).'-'.time()]); \App\Models\Translation\BlogCategoryTranslation::updateOrCreate(['blog_category_id'=>$cat->id,'locale'=>mb_strtolower($data['locale']??app()->getLocale())],['title'=>$data['title']]); return redirect()->route('panel.v1.admin.marketing.blog-categories')->with('toast',['title'=>'تم','msg'=>'تم الإنشاء','type'=>'success']); }
    ...
    public function createTestimonial(Request $request){ $u=$this->resolveAdmin($request); if($u instanceof \Illuminate\Http\RedirectResponse) return $u; return $this->renderAdmin($request,'panel_v1.admin.pages.marketing.testimonial-form','رأي جديد',array_merge(AdminMockData::shell('marketing','testimonials'),['formAction'=>route('panel.v1.admin.marketing.testimonials.store')])); }
    public function storeTestimonial(Request $request){ $u=$this->resolveAdmin($request); if($u instanceof \Illuminate\Http\RedirectResponse) return $u; $request->validate(['title'=>'required','content'=>'required']); $data=$request->all(); if(class_exists(\App\Models\Testimonial::class)) \App\Models\Testimonial::create(['title'=>$data['title'],'content'=>$data['content'],'image'=>$data['image']??null,'created_at'=>time()]); return redirect()->route('panel.v1.admin.marketing.testimonials')->with('toast',['title'=>'تم','msg'=>'تم الإنشاء','type'=>'success']); }
